...
def get_tasks():
    ...
    try:
        ...
        dead_tasks = bot.task_queue.get_dead_tasks(limit=50)
        return jsonify({
            "code": 0,
            "message": "success",
            "data": {
...
@bp.route("/api/tasks/dead", methods=["GET"])
@ws.login_required
def get_dead_tasks():
    from flask import request, jsonify
    try:
        ...
        try:
            ...
            limit = 50
        tasks = ws.bot.task_queue.get_dead_tasks(limit=limit)
        tasks_data = [task.to_dict() for task in tasks]
        return jsonify({"code": 0, "message": "success", "data": tasks_data})
    except Exception as e:
        ws.log("ERROR", f"获取死信任务列表失败: {e}")
        return jsonify({"code": 500, "message": str(e), "data": None}), 500


@bp.route("/api/tasks/recover", methods=["POST"])
@ws.login_required
def recover_task():
    from flask import request, jsonify
    try:
        ...
        if not task_id:
            return jsonify({"code": 400, "message": "task_id 参数不能为空", "data": None}), 400
        success = ws.bot.task_queue.recover_task(task_id)
        if success:
            return jsonify({"code": 0, "message": "任务已恢复，重新加入待执行队列", "data": {"task_id": task_id}})
        return jsonify({"code": 404, "message": "任务不存在或不在死信队列", "data": None}), 404
    except Exception as e:
        ws.log("ERROR", f"恢复死信任务失败: {e}")
        return jsonify({"code": 500, "message": str(e), "data": None}), 500


@bp.route("/api/tasks/discard", methods=["POST"])
@ws.login_required
def discard_task():
    from flask import request, jsonify
    try:
        ...
        if not task_id:
            return jsonify({"code": 400, "message": "task_id 参数不能为空", "data": None}), 400
        success = ws.bot.task_queue.discard_task(task_id)
        if success:
            return jsonify({"code": 0, "message": "死信任务已丢弃", "data": {"task_id": task_id}})
        return jsonify({"code": 404, "message": "任务不存在或不在死信队列", "data": None}), 404
    except Exception as e:
        ws.log("ERROR", f"丢弃死信任务失败: {e}")
        return jsonify({"code": 500, "message": str(e), "data": None}), 500